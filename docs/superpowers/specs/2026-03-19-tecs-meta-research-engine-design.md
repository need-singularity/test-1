...
```
fitness = w_e * emergence_score + w_b * benchmark_score + w_f * efficiency_score

emergence_score = normalize(β_change + χ_change + r_sync + Φ_integration + λ_transition)
  각 지표는 [0, 1]로 min-max 정규화 (슬라이딩 윈도우 기반)

benchmark_score = (concept_acc + contradiction_prec + analogy_acc) / 3

efficiency_score = 1.0 - normalize(total_compute_cost)

기본 가중치: w_e = 0.4, w_b = 0.4, w_f = 0.2
config.yaml에서 조정 가능
```

### 6.2 세대 루프

1. **선택**: 토너먼트 선택 (tournament_size=3), 상위 20% 엘리트 보존, population_size=50
...
- Δmetric_i / Δcomponent_j → 인과 강도 행렬 (편미분 근사)
- 다수 세대 누적 → 통계적 유의성 검증 (p < 0.05)
- **비용**: 추가 시뮬레이션 0회 (이미 실행된 데이터만 분석)
- **한계**: 진정한 ablation 대비 인과 추론 강도가 약함. 교란 변수(confounders) 가능. 충분한 세대 수(≥20) 누적 후 신뢰도 향상
...
| 지표 | 계산 방법 | 급등 판정 | 의미 |
|------|----------|----------|------|
| β (베티수) | GUDHI persistent homology | \|Δβ_n\| > 2σ (슬라이딩 윈도우 10세대) | 새 위상 구조 출현 |
| χ (오일러 특성) | Σ(-1)^n · β_n | \|Δχ\| > 2σ (슬라이딩 윈도우 10세대) | 전역 위상 복잡도 변화 |
| r (순서 매개변수) | 쿠라모토 order parameter | Δr/Δt > 0.2 per generation | 동기화 상전이 |
| Φ (통합 정보) | 근사: atomic Φ (Oizumi 2014) | Φ > 1.0 | 정보 통합도 임계 초과 |
| λ (리아프노프) | 궤적 발산률 수치 추정 | λ: 음→양 전환 | 혼돈 진입 (분기점) |

**윈도우 및 활성화:** σ는 최근 10세대의 슬라이딩 윈도우로 계산. 세대 < 10일 때는 전체 히스토리 사용. 세대 < 3일 때는 급등 감지 비활성화.

**Φ 계산 비용 대응:** 10² 노드에서는 atomic Φ 근사 (서브그래프 분할), 10³ 이상에서는 stochastic Φ 추정 (Monte Carlo 샘플링). 정확한 Φ는 10⁴ 노드에서 계산 불가능하므로 항상 근사 사용.
...
| 4→2 | 인과 변이 후보의 best_fitness > 이전 루프의 best_fitness + 0.01 | 변이 후보로 Phase 2 재진입 |
| 4→5 | Phase 4→2 루프 ≥ 10회, 또는 best_fitness 개선 ≤ 0.01 | 최적 후보로 대규모 검증, 스케일 10³→10⁴ |
...
| 태스크 | 외부 소스 | 자체 생성 | 크기 |
|--------|----------|----------|------|
| 개념관계 추론 | ConceptNet 5.7 관계 트리플 (로컬 캐시) | 지식 그래프에서 1-hop/2-hop 자동 추출 | 외부 1000개 + 자체 500개 |
| 모순 탐지 | ConceptNet NotDesires/NotCapableOf 관계 | 긍정 트리플의 관계를 반전 (IsA→NotIsA) | 외부 500쌍 + 자체 500쌍 |
| 유추 추론 | Turney 2006 SAT analogy corpus | 위상 서명 유사 구조쌍 자동 추출 | 외부 374문항 + 자체 300문항 |

**데이터 파이프라인:**
- ConceptNet: 초회 실행 시 API에서 다운로드 → `data/conceptnet/` 로컬 캐시 (이후 오프라인 사용)
- WordNet: NLTK 데이터 패키지 (자동 다운로드)
- SAT corpus: Turney 2006 공개 데이터셋 (374문항)
- 자체 생성 부정 변환: 관계 타입별 규칙 기반 (IsA→NotIsA, HasA→LacksA 등 매핑 테이블)
- 자체 생성 유추: 동일 호몰로지 서명을 가진 구조쌍을 지식 그래프에서 탐색
...
ComplexType = Union[gudhi.SimplexTree, nx.Graph, "HyperNetXHypergraph"]

@dataclass
class TopologyState:
    complex: ComplexType       # 구조체 (타입 명시)
    complex_type: str          # "simplicial" | "graph" | "hypergraph"
    curvature: np.ndarray      # 곡률 텐서 (이산 근사)
    metrics: dict[str, float]  # 현재 지표
    history: list              # 변형 경로 기록
    metadata: dict             # 추가 정보
```

### 12.2 Component Protocol (기본)

```python
class Component(Protocol):
    name: str
    layer: str
    compatible_types: list[str]  # ["simplicial", "graph"] 등

    def configure(self, params: dict) -> None:
        """파라미터 주입 (스텝 수, 온도 스케줄 등)"""
        ...

    def execute(self, state: TopologyState) -> TopologyState:
        """입력 상태를 받아 변환된 상태를 반환"""
        ...
    ...
    def cost(self) -> float:
        """상대적 계산 비용 (0.0~1.0, 자원 배분용)"""
        ...
```

### 12.3 VerificationComponent Protocol (검증 계층 전용)
...
persistent_homology_dual, shadow_manifold_audit, stress_tensor_zero는 이 프로토콜을 구현한다. `reference`는 시뮬레이터가 자동으로 듀얼 상태를 생성하여 전달한다.

### 12.4 타입 호환성 체크

`TopologySimulator`가 파이프라인 실행 전에 각 구성요소의 `compatible_types`와 `TopologyState.complex_type`을 대조하여, 비호환 조합은 사전 거부한다.
...
| 프로세스 크래시 | 매 세대 완료 시 `checkpoint.json` 저장. 재실행 시 `--resume` 플래그로 마지막 체크포인트에서 복구 |
...
```
results/runs/run_*/checkpoint.json
{
  "phase": 2,
  "generation": 34,
  "population": [...],
...
매 세대 완료 시 자동 저장. `python run.py --resume results/runs/run_20260319_143022/` 로 복구.

---

## 16. 후보 계보 (Candidate Lineage)

각 후보 아키텍처에 고유 ID와 부모 정보를 부여하여 계보를 추적한다:

```python
@dataclass
class Candidate:
    id: str                    # uuid4
    components: dict[str, str] # {"repr": "simplicial_complex", ...}
    parent_ids: list[str]      # 부모 후보 ID (교차 시 2개, 변이 시 1개)
    generation: int
    phase: int
    fitness: float
    mutation_layer: str | None # 변이된 계층 (인과 분석 연계)
```

이를 통해 CausalTracer가 "어떤 조상에서 어떤 변이가 성능 향상을 일으켰는지" 계보 기반 추적이 가능하다.
